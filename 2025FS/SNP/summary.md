...
- [ ] (void) when ignoring return value
- [ ] void when no function parameters
- [ ] const on read only parameters
- [ ] return EXIT_SUCCESS / EXIT_FAILURE instead of integer
- [ ] error handling for everything (especially with IO functions like fopen)
- [ ] type cast on malloc and void pointers
- [ ] use size of type in malloc length (= malloc(some length * sizeof(some type))
- [ ] include everything (eg stdio for printf)
- [ ] in code explanation: explain every word
- [ ] use free() after malloc
- [ ] use %x in scanf
- [ ] think of null pointer at the end of a string
- [ ] check if malloc result is not NULL
- [ ] if(x = 0) is wrong, use ==
...
- constant data types -----> depends on the implementation. each option has its own read only segment for constants
...
##### Const
- automatic variable allocated on the stack
- should be used on read only function parameters as often as possible
- an array cant be constant, const int means that each integer in the array is constant
- 
##### Second Last Digit
```c
int z = 123;
...
### Pointers
Addition and substraction allowed, but not multiplication or division.
Addition of 2 Pointers not allowed.
...
##### Mutex
[10_SNP_Sys_Thread_Synchronisation-mit-Notizen](2025FS/SNP/script/10_SNP_Sys_Thread_Synchronisation-mit-Notizen.pdf)
```c
#include <sys/types.h>
#include <pthread. h>
#include <stdlib.h>
...
- Links to the path of the file
- If the file was deleted, the link points to nothing
```
ln -s a b
```
...
- Both file names have the same importance (there is no "original file" anymore)
- If you delete one of the files, the file will stay, as long as there is at least one hard link.
```
ln a b
```
