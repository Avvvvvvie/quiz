Ein Teil der Fragen stammt von https://www.studocu.com/de-ch/document/zurcher-hochschule-fur-angewandte-wissenschaften/algorithmen-und-datenstrukturen/sep-fragen/39612610?origin=course-highest-rated-2
### Nenne ein Beispiel für eine Vor- und Nachbedingung.
Beispiel: {x=1} x++ {x=2}
...
### Was ist eine Queue, wie kann man sie implementieren und was hat sie für Methoden?
First in Last out
...
### Wie funktioniert die ArrayQueue
von outIndex bis inIndex

wenn bei array.size dann wraparound

### Was hat die Klasse Iterator für Methoden?
- hasNext
- next
- remove

### Was sind 4 Vorteile vom Iterator?
- Hiding-Prinzip wird nicht verletzt
- Mehrere gleichzeitig iterierbar
- Verwaltet aktuelle Position
- foreach tut dies

### Wann benutzt man ArrayList, wann List?
arraylist -oft zugreifen

list -oft verändern

### Wie entfernt man ein Element der LinkedList
a.next = a.next.next

### Wie fügt man eine Element zu LinkedList hinzu?
newnode.next = listnode1.next

listnode1.next = newnode

### Laufzeit Einfügen an der ersten/i-ten Position in einer einfach verketteten Liste?
O(1) und O(n)

### Was ist das Prinzip der Doppelt verketteten Liste?
Hat head und Tail. Jeder Knoten zeigt auf den vorherigen und den nächsten.

### Wie sieht der Code von Add aus bei einer doppelt verketteten Liste
...
### Welche Resultate kann es bei x.compareTo(y) geben?
x < y -negativ
...
### Was ist der Unterschied von einem Array und einer Liste?
Array: get() effizient
...
### Welche Methoden besitzt Collections?
- add
- remove
- removeAll
- toArray
...
### Was sind die Vorteile von Java Generics?
...
### Wie sieht ein Generic Wildcard aus?
...
- new e() nicht möglich
deshalb: `Class<Tclazz -(T)clazz.newInstance()` um Klasse herauszufinden

### Gib ein Code Beispiel für die Nutzung der Klasse Predicate?
predicate = x -x 1

predicate.test(5)

### Was ist der Ablauf beim Nutzen von Streams?
- Erzeuge Stream
- Verarbeite Stream (Intermediate Operations)
- Konsumierte Stream (Terminate)
- Kaskadiert und Parallel

### Wie sieht der Code zur Fakultät rekursiv aus?
```
int fak(int n) {
   if (n == 0) return 1;
...
### Wie sieht der Code für Fibanocci rekursiv aus?
```
int fib(int n) {
   if (n == 0 || n == 1) return 1;
...
### Was ist die Laufzeit von Fibonacci?
O(1.6^n)

### Was ist die Definition von Rekursion?
...
### Wie ist einer Liste rekursiv definiert?
Liste = leer

Liste = ListNode (Liste)?

### Wie sieht der Code für traverse() einer Liste rekursiv aus?
```
if p !== null {
   print(p.value)
   traverse(p.next)
...
### Nenne zwei Implementierungen von Listen in Java
ArrayList, LinkedList

### Was ist der Unterschied von direkter und indirekter Rekursion?
Direkt: p() enthält p()
...
### Nennen Sie ein Beispiel rekursiver Programmierung aus der Vorlesung.
Fibonacci, isPrime, Fakultät, Liste/Baum traversieren 

### Schreiben Sie einen Algorithmus in Pseudo-Code, der überprüft, ob eine Zahl eine Primzahl ist.
Iterativ:
```
for i=n, i>1,i--
...
### Was hält die Omega-Notation fest?
Wie lange der Algorithmus mindestens braucht in ABhängigkeit des Inputs.

### Was hält die Theta-Notation fest?
Exakte Wachstumsrate (statt Best/Worst Case). x^2 ist auch O(x^3) aber nicht Theta(x^3)

### Laufzeit folgendes Codes: `while i < N: i = i*2`
log(n)

### Laufzeit folgendes Codes: `for i = 1 .. n; for j = 1 .. n`
O(n^2)

### Laufzeit folgendes Codes: `for i = 1..n; for j = 1..i for k = 1..j`
for 1..n -n
...
### Laufzeit folgendes Codes: `for i = 1..n; binarySearch(i in F)`
n log(n)

### Laufzeit folgendes Codes: `for i = 1..n for j = 1..m for k = 1..m`
O(n * m^2)
...
### Falls immer f(x) < g(x) ist, ist dann f = O(g)?
Ja, weil laut Definition f(x) O(g(x)) ist wenn ab einem gewissen Wert f(x) <= c * g(x) gilt.

### Warum sind iterative Algorithmen oft schneller als ihre rekursiven Varianten?
Overhead beim Methodenaufruf

### Was ist die Komplexität von Turm von Hanoi und wie könnte der Code aussehen?
...
### Was ist ein binaerer Suchbaum? Was ist ein binärer Baum?
...
### Wie ist ein Binärer Baum rekursiv definiert?
Baum = leer

Baum = Knoten (Baum Baum)

### Welche Traversierungsarten bei Bäumen kennen Sie?
Preorder: n, A, B
...
### Pre-/In-/Postorder Implementation mit Rekursion
```
if node != null
	visit node
	preorder(left);
...
### Wie könnte man Pre-/In-/Postorder mit einem Stack implementieren?
```
enqueue root
...
### Wie sieht eine Levelorder Implementation aus?
```
enqueue root
...
### Was passiert, wenn Sie die Knoten eines binaeren Suchbaums inorder ausgeben?
Die Elemente werden geordnet ausgegeben.

### Gegeben sei ein Baum, bei dem die inneren Knoten Operationen sind wie + - * / und die Blaetter Zahlen. Was erhalten Sie bei Inorder,Preorder Postorder?
Inorder: a + b
...
### Fügen Sie die Zahlen 10, 1, 9, 2, 8, 3, 7, 4 der Reihe nach in einen binaeren Suchbaum ein. Was erkennen Sie? 
Zickzack

### In einem bin. Suchbaum mit ungerade vielen Elementen (n = 2k+1) steht an jedem Knoten, wieviele Knoten der gesamte Teilbaum hat. Wie kann man mit dieser Information den Median finden?
Baum traversieren und Knoten finden, bei welchem gleich viele Knoten links und rechts sind.

### Wo findet man in einem binären Suchbaum den grössten Knoten?
Ganz rechts

### Wie hoch ist ein binaerer Baum mit n Elementen?
mindestens log2(n + 1)

höchstens n

### Wie viele Knoten kann ein Baum der Höhe k haben?
2^k - 1

### Skizzieren Sie einen vollständig gefüllten, binären Suchbaum der Höhe 3 und löschen Sie anschliessend den gezeigten Knoten aus dem Baum. Kann man aus der Pre- und Postorder-Reihenfolge einen Baum immer eindeutig wiederherstellen?
Nein. Nur wenn man auch Inorder hat. Auch Inorder+Pre ohne Post

### Wie kann man einen Node einfügen unsortiert? (insertAt)
```
insertAt(node, x) {
	if node = null
...
### Wie kann man einen Node einfügen sortiert (insertAt)
```
insertAt(node, x) {
	if node == null
		return new TreeNode(x)
...
### Wie ist die Worst-Case-Laufzeit von binärer Suche? Wieso?
...
### Welche Laufzeit hat die Suche in einer unsortierten LinkedList? Was ist, wenn die Liste sortiert ist?
Beides O(n)

### Wie läuft das Löschen eines Teilbaums aus?
...
### Was ist die AVL Bedingung?
- Tiefe von 2 Teilbäumen maximal 1 Unterschied = AVL-Balanciert
- Super Balanciert *wäre* Gewicht max 1 Unterschied

### Wa sind die Vor- und Nachteile von AVL
- Schnelle Suche O(log n)
- Wird nicht zu Liste
- Mehr Aufwand

### In welchem Fall gibt es welche AVL Rotation?
Einzelrotation: Wenn L tiefer: L wird oben, obenlinks = Lrechts

Doppelrotation: Wenn mitte links tiefer->LR

### Ein AVL-Baum sei durch Einfügen eines Elements aus der Balance geraten. Wie kann man erkennen, ob eine einfache oder eine doppelte Rotation gebraucht wird, um die Balance wiederherzustellen?
...
### Was sind die Vorteile von einem B-Baum?
...
### Wie unterscheiden sich B-Baum und AVL?
...
### Wieviele Knoten hat ein B-Baum der Ordnung k und Höhe h höchstens?
k^(h-1) + (k-1)

### Wieviele Kanten hat ein vollständiger (kompletter) ungerichteter Graph mit n Knoten?
n^2

### Was ist der Unterschied zwischen einem dichten und einem dünnem Graph?
Dicht: Viele Kanten
Dünn: Wenige Kanten

### Wie kann man einen ungerichteten Graphen als gerichteten Graphen repräsentieren?
...
### Wie sieht der Code für Breitensuche aus?
```
mark startNode
...
### Wie sieht der Code für Tiefensuche aus?
```
mark startNode
...
### Wie findet man den Kürzester Pfad?
...
### Wie funktioniert der Dijkstra-Algorithmus? Wie sieht der Code aus?
...
### Wie lautet die Definition eines ungerichteten (gerichteten, gewichteten) Graphen?
...
### Sind minimale Spannbaeume eindeutig bestimmt? Begruendung/Gegenbeispiel
Nein. Wenn zwei Wege gleich lang die zwei Teilbäume verbinden würde an anderen Orten.

### Angenommen es gibt in einem Graphen genau eine Kante minimalen Gewichts. Muss die in einem minimalen Spannbaum vorhanden sein? Begruendung 
...
### Gegeben ist ein Spannbaum eines ungerichteten Graphen. Nun verbinden Sie zwei bisher nicht verbundene Knoten. Ist das Resultat immer noch ein Spannbaum?
Nein. Dadurch entsteht ein Zyklus und dadurch ist es kein Baum mehr.

### Welche Graphenrepräsentation würden Sie verwenden, um einen Graphen als Textfile zu speichern?
Adjazenzliste wenn mehr Knoten oder Matrix wenn mehr Kanten und gewichtet

### Sind Spannbäume eindeutig bestimmt? Wenn ja, begründen Sie, wenn nein geben Sie ein Gegenbeispiel.
Nein, wenn es Pfade gibt mit der gleichen Gewichtung.

### Sie haben einen unger. Graphen mit den vorgegebenen Kanten. Finden Sie eine geschlossene Tour (Anfangsknoten = Zielknoten), bei der jede Kante genau einmal genommen wird, oder begruenden Sie, warum es keine solche Tour geben kann.
Es geht nicht, wenn ein Knoten eine ungerade Anzahl Kanten hat. Wo man hingeht muss man nämlich wieder weggehen können.

### Traveling Salesman Problem: Was ist das Problem und was ist die Laufzeit?
...
### Wie sieht rekursive Suche im Labyrinth aus?
Nutzt Backtracking
```
...
### Wie löst man das Springerproblem?
...
### Wie löst man das 8 Damenproblem?
...
### Warum benutzt man eine Zielfunktion und wie funktioniert sie?
...
### Wie findet man den besten Zug in Tic-Tac-Toe?
...
### Wie könnte man Binäre Suche implementieren?
- m = (l + r) / 2
- Falls a[m] < S -l = m
- Falls a[m] S -r = m
...
### Was ist der Aufwand der Suche in 2 sortierten Arrays und wie geht man bei der Implementierung vor?
Aufwand: O(n)
```
...
### Was muss man in Java beachten wenn man selber hashen möchte?
...
### Wann gibt es viele Kollisionen?
Wenn der Load-Faktor lambda 0.8 dann gibt es viele Kollisionen

### Was gibt es für Methoden zur Kollisionsauflösung beim Hashing?
...
### Was sind die Vor- und Nachteile von Hashing?
- suchen, einfügen effizient
- alles mit ordnung, bereichen ineffizient

### Platz in Hashtabelle reicht nicht mehr was tun?
...
### Welche Methoden haben Map und HashMap?
- put
- get
- remove
- size
...
### Wie unterscheiden sich Bruteforce und KMP?
O(n * m) VS O(n + m)
...
### Vergleiche alle Sortieralgorithmen nach best/worst/avg Laufzeit. Welche benötigen mehr/weniger Swaps? Welche profitieren von einer Vorsortierung?
...
### Welchen Sortieralgorithmus soll man wann nutzen?
- Wenige Daten:
...
### Nennen Sie für vier Komplexitätsklassen (O-Notation) einen Algorithmus mit dieser Laufzeit
- O(1): Array get()
- O(n): List get()
- O(log n): Binary Search AVL Baum
- O(n log n): Quicksort

### Wie lautet die Rekursionsgleichung für die maximale Anzahl Vergleiche bei binärer Suche?
$T(n) = T(n/2) + 1$

### Was sind die Vorteile und Nachteile eines static imports?
