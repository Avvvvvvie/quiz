...
void handleRequestPers(char* requestBuffer, int requestLen)
{
    ...
    switch(cmd)
    {
    	...
    	case 'C' :
    	{
			printf("handleRequestPers: clear\n") ;
            list_clear();
			break;
		}
    	case 'S':
		...
		break;
     	...
     	default:
		{
 			printf("handleRequestPers, invalid command cmd=%c\n",cmd);
		}
		break;
    }
}
...
void client_main(int socket) {
	...
	while (shutdown == 0) {
		int requestLen = getRequest(buffer, MAX_LEN);
		handleRequestPers(buffer, requestLen);
	}
}
...
int main(int argc, char* argv[])
{
	...
	while (shutdown == 0) {
		int clientSocket = wait_client();
		client_main(clientSocket);
	}
